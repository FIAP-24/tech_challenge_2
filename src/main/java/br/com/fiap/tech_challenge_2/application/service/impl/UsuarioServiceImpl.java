...
public class UsuarioServiceImpl implements UsuarioService {
    ...
    public UsuarioResponse save(UsuarioRequest request) {
        TipoUsuario tipoUsuario = validPerfil(request.perfil());
    ...
    @Override
    public void alterPassword(Long id, UsuarioPassRequest usuarioPassRequest) {
        if(!usuarioPassRequest.senhaNew().equals(usuarioPassRequest.senhaConfirm())){
            throw new AuthenticationException("Senha nova e confirmação devem ser iguais");
        }
        Usuario usuario =
                usuarioRepository
                        .findByLogin(usuarioPassRequest.login())
                        .orElseThrow(() -> new AuthenticationException("Login não encontrado"));

        boolean authenticated = passwordHasher.verifyPassword(usuarioPassRequest.senhaOld(), usuario.getSenha());
        if (!authenticated) {
            throw new AuthenticationException("Senha incorreta");
        }
        usuario.setSenha(passwordHasher.hashPassword(usuarioPassRequest.senhaNew()));
        usuarioRepository.save(usuario);
    }

    private TipoUsuario validPerfil(String request) {
        return tipoUsuarioRepository.findByNome(request)
                .orElseThrow(() -> new ResourceNotFoundException("Tipo de usuário não encontrado: " + request));
    }
